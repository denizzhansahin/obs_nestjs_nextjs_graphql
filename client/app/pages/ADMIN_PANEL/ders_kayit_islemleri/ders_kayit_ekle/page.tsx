...
import { CREATE_ENROLLMENT } from '@/app/GraphQL/DersKayitSorgu';
...
export default function DersKayitEkle() {
  ...
  const [createEnrollment, { loading, error, data }] = useMutation(CREATE_ENROLLMENT);
      ...
      const { data } = await createEnrollment({
        variables: {
          createdEnrollmentsData: {
            student_id: parseFloat(userId_st as string),
            course_id: parseFloat(courseId as string),
            academician_id: userId ? parseFloat(userId as string) : null,
            enrollment_date: enrollment_date ? enrollment_date?.toISOString() : null,
            status: status ? status : "null"
          },
        },
      });
      alert(`Ders kayıt oluşturuldu: ${data.createInstructors.first_name}`);
    } catch (error) {
      console.error("Ders kayıt oluşturma hatası:", error);
    }
  };
